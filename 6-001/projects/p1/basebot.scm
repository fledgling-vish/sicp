...
(define time-to-impact
  (lambda (vertical-velocity elevation)
    (define t1 (root1 (- 4.9) vertical-velocity elevation))
    (if (> t1 0)
	t1
...
(define travel-distance-simple
  (lambda (elevation velocity angle)
    (define ar (degree2radian angle))
    (define vx (* velocity
		  (cos ar)))
    (define vy (* velocity
		  (sin ar)))
    (define t (time-to-impact vy elevation))
    (* t vx)))
...
;; at an angle of 0 (straight horizontal) 20.3 m 
;; at an angle of (/ pi 2) radians or 90 degrees (straight vertical) 0 m
;; at an angle of (/ pi 4) radians or 45 degrees 207.6 m 
...
(define find-best-angle
  (lambda (velocity elevation)
    (define max-angle 90)
    (define (t next-angle best-range-seen best-angle-seen)
      (define range (travel-distance-simple elevation velocity next-angle))      
      (cond ((> next-angle max-angle) best-angle-seen)
	    ((> range best-range-seen) (t (+ next-angle 1) range next-angle))
	    (else (t (+ next-angle 1) best-range-seen best-angle-seen))))
    (t 1 (travel-distance-simple elevation velocity 0) 0)))
...
(define travel-distance
  (lambda (h V alpha)
    (integrate 0 h (* V (cos alpha)) (* V (sin alpha)) 0.01 9.8 mass beta)))
